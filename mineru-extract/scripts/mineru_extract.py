...
def extract_markdown_from_zip(zip_bytes: bytes, out_dir: pathlib.Path) -> tuple[pathlib.Path | None, list[pathlib.Path]]:
    ...
    # prefer a "main" markdown: largest non-debug
    def score(p: pathlib.Path) -> tuple[int, int]:
        name = p.name.lower()
        penalty = 0
        # heuristics: deprioritize debug/aux
        if "readme" in name:
            penalty += 2
...
def main() -> int:
    ...
    if not model_version:
        # Heuristic: HTML if not obvious doc extension
    ...
    base_out = pathlib.Path(args.out) if args.out else (pathlib.Path("/home/node/.openclaw/workspace") / "mineru" / sanitize_filename(task_id))
    ...
    # Always print a machine-readable summary to stderr; stdout reserved for markdown when --print
    summary = {
        "task_id": task_id,
        "state": data.get("state"),
        "model_version": model_version,
